...
<body class="sanckbg mode_webapp">
	<div id="menu-container" style="display: block;">
		<div class="menu_header">
			<div class="menu_topbar">
				<div id="menu" class="sort sort_on">
					...
					<ul>
						<?php if(is_array($menu)): $i = 0; $__LIST__ = $menu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menuid): $mod = ($i % 2 );++$i;?><li><a href="javascript:showProducts('<?php echo ($menuid["id"]); ?>')"><?php echo ($menuid["value"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
		...
		<div class="gonggao">
			...
			<div class="content"><?php echo ($info["gonggao"]); ?></div>
		</div>
		...
		<section class="menu">
						...
						<?php if(is_array($goods)): $i = 0; $__LIST__ = $goods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goodsvo): $mod = ($i % 2 );++$i;?><dd menu="<?php echo ($goodsvo["menu_id"]); ?>">
							<div class="tupian">
								<img src="__PUBLIC__/Uploads/<?php echo ($goodsvo["image"]); ?>"
									...
									href="javascript:doProduct('<?php echo ($goodsvo["id"]); ?>','<?php echo ($goodsvo["title"]); ?>','<?php echo ($goodsvo["price"]); ?>');" class="add"><p
										class="dish2"><?php echo ($goodsvo["title"]); ?></p>
									<p class="price2"><?php echo ($goodsvo["price"]); ?>元/份</p>
									<p>
										<del><?php echo ($goodsvo["old_price"]); ?>元/份</del>
									</p></a>
							</div>
							<a href="javascript:doProduct('<?php echo ($goodsvo["id"]); ?>','<?php echo ($goodsvo["title"]); ?>','<?php echo ($goodsvo["price"]); ?>');" id="<?php echo ($goodsvo["id"]); ?>" class="reduce" style="display: block;"><b class="ico_reduce">减一份</b></a>
			...
			<div id="mcover"
				...
				<a class="close"
					onclick="document.getElementById('mcover').style.display='';">X</a>
			</div>
